...
SSH_TARGET=""
SSH_LOGIN_USER=""
SSH_CONTROL_PATH="${TMPDIR:-/tmp}/grid34-ssh-%C"
...
render_header() {
	...
	printf '\n'
	hr
	panel_line "$(status_info "grid34 deploy installer")"
	hr
...
Options:
  ...
  --install-dir <path>     Remote install path (default: /var/www/grid34)
...
Environment overrides:
  GRID34_SSH_TARGET, GRID34_PORT, GRID34_INSTALL_DIR,
  GRID34_SERVICE_USER, GRID34_SERVICE_GROUP, GRID34_PROXY,
  GRID34_DOMAIN, GRID34_CADDY_EMAIL, GRID34_SSH_PORT,
  GRID34_DRY_RUN, GRID34_SKIP_BUILD
EOF
}
...
managed_marker="# managed by grid34"
...
printf '\n'
hr
panel_line "grid34 remote installer"
hr
panel_line "install  : ${INSTALL_DIR}"
panel_line "service  : ${SERVICE_NAME}"
panel_line "port     : ${PORT}"
...
detect_caddy_snippet_dir() {
	...
	local dir
	if [[ -n "${GRID34_CADDY_SNIPPET_DIR:-}" ]]; then
		printf '%s' "$GRID34_CADDY_SNIPPET_DIR"
		return 0
	fi
	if [[ -f "$main_file" ]]; then
...
${import_line}"
		...
		if grep -qE '^[[:space:]]*import[[:space:]].*(/etc/caddy/)?(conf\.d|Caddyfile\.d)/\*\.caddy' "$main_file"; then
			...
			log "existing Caddyfile is managed by grid34; adding snippet import"
...
detect_nginx_style() {
	...
	if [[ -n "${GRID34_NGINX_STYLE:-}" ]]; then
		printf '%s' "$GRID34_NGINX_STYLE"
		return 0
	fi
...
SSH_TARGET="${GRID34_SSH_TARGET:-$SSH_TARGET}"
PORT="${GRID34_PORT:-$PORT}"
INSTALL_DIR="${GRID34_INSTALL_DIR:-$INSTALL_DIR}"
SERVICE_USER="${GRID34_SERVICE_USER:-$SERVICE_USER}"
SERVICE_GROUP="${GRID34_SERVICE_GROUP:-$SERVICE_GROUP}"
PROXY_MODE="${GRID34_PROXY:-$PROXY_MODE}"
DOMAIN="${GRID34_DOMAIN:-$DOMAIN}"
CADDY_EMAIL="${GRID34_CADDY_EMAIL:-$CADDY_EMAIL}"
SSH_PORT="${GRID34_SSH_PORT:-$SSH_PORT}"

if is_true "${GRID34_DRY_RUN:-false}"; then
	DRY_RUN=true
fi
if is_true "${GRID34_SKIP_BUILD:-false}"; then
	SKIP_BUILD=true
fi

[[ -n "$SSH_TARGET" ]] || die "an SSH host is required; pass --host user@server or set GRID34_SSH_TARGET"
if [[ -t 0 && -t 1 ]]; then
	TTY_AVAILABLE=true
else
	warn "terminal is not interactive; SSH password prompts may not be available"
fi
case "$PROXY_MODE" in
